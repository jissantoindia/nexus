...
// ─── Project-level generated documentation (cloud-backed) ────────────────────
// Stores the full project docs JSON in the settings collection so collaborators
// can also see/access docs generated by the project owner.
// Document ID = `pdoc_<projectId>` (stable, upsert-friendly)

export async function getProjectDocsCloud(projectId) {
  const docId = `pdoc_${projectId}`;
  try {
    const raw = await databases.getDocument(DB_ID, SETTINGS_COL, docId);
    return JSON.parse(raw.release_notes); // re-use the large text field
  } catch {
    return null; // not yet generated
  }
}

export async function saveProjectDocsCloud(projectId, docData) {
  const docId  = `pdoc_${projectId}`;
  ...
  const payload = {
    ...
    mac_url:       '',
    linux_url:     '',
    release_notes: json.slice(0, 900000),
  };
  try {
    await databases.getDocument(DB_ID, SETTINGS_COL, docId);
    return databases.updateDocument(DB_ID, SETTINGS_COL, docId, payload);
  } catch {
    return databases.createDocument(DB_ID, SETTINGS_COL, docId, payload);
  }
}
